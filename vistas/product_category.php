...
<div class="container pb-6 pt-6">
   ...
   <div class="columns">
      ...
      <div class="column is-one-third">
         ...
         if ($categorias->rowCount() > 0) {
            ...
            foreach ($categorias as $row) {
               echo '<a href="index.php?vista=product_category&category_id=' . $row['id_categoria'] . '" 
               class="button is-link is-inverted is-fullwidth">' . $row['nombre_categoria'] . '</a>';
            }
         } else {
      ...
      <div class="column">
         ...
         $categoria = conexion();
         $categoria = $categoria->query("SELECT * FROM categoria WHERE id_categoria = '$id_categoria'");

         if ($categoria->rowCount() > 0) {
            $categoria = $categoria->fetch();

            echo '<h2 class="title has-text-centered">' . $categoria['nombre_categoria'] . '</h2>
            <p class="has-text-centered pb-6">' . $categoria['ubicacion_categoria'] . '</p>';
